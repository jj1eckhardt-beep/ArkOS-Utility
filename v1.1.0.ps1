...
function Start-FileTransfer {
    ...
    foreach ($file in $processedFiles) {
        ...
        if (!(Test-Path $targetFile) -or ($file.LastWrite -gt (Get-Item $targetFile).LastWriteTime)) {
            ...
            # Flags Explained:
            # /J    = Unbuffered I/O (Prevents large files from freezing the OS)
            # /MT:8 = Multi-threaded (Faster for small files)
            # /Z    = Restartable mode (Great for SD cards)
            # /NP   = Suppress progress clutter (Lets our custom UI handle it)
        ...
        # --- PROGRESS UPDATE ---
        $bytesDone += $file.Size
        $global:CurrentBytes = $bytesDone # This feeds the Background Timer math
...
function Show-HelpManual {
    ...
    $HelpItems = @(
        ...
        @{ Cmd = " (System Only)" ; Desc = "Clones BIOS, Ports, and Tools folders only." },
        ...
        @{ Cmd = "[ ABORT ]"      ; Desc = "Sends the ABORT command and immediately halts the current operation." },
        @{ Cmd = "[ GEN xxxOS ]"  ; Desc = "Creates the specific folder structure for ArkOS / dArkOS / Rocknix." },
